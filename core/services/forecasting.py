...
class TimeSeriesForecaster:
    """
    Monthly forecasting only.

    Rules:
    - Ignore latest available month(s) from training
    - Train on previous complete months only
    - Use last N years of history
    - Forecast starting from ignored month
    ...
    def _build_monthly_series(self, metric="visits"):
        ...
        if metric == "patients":
            monthly = (
                df.groupby("month_start")["patient_id"]
                .nunique()
                .reset_index(name="value")
            )
        elif metric == "hospitals":
            monthly = (
                df.groupby("month_start")["hospital_id"]
                .nunique()
                .reset_index(name="value")
            )
        elif metric == "avg_cost":
            monthly = (
                df.groupby("month_start")["total_claim_cost"]
                .mean()
                .reset_index(name="value")
            )
        elif metric == "avg_coverage":
            monthly = (
                df.groupby("month_start")["payer_coverage"]
                .mean()
                .reset_index(name="value")
            )
        elif metric == "avg_oop":
            monthly = (
                df.groupby("month_start")["out_of_pocket"]
                .mean()
                .reset_index(name="value")
            )
        else:
        ...
        if len(unique_months) < self.ignore_recent_months:
            forecast_start_month = pd.Timestamp(unique_months[0])
            return pd.DataFrame(columns=["month_start", "value"]), forecast_start_month
        ...
        min_train_month = last_train_month - pd.DateOffset(years=self.years_back)
    ...
    def _recursive_forecast(self, monthly_train, model, feature_cols, forecast_start_month):
        hist = (
            monthly_train[["month_start", "value"]]
            .copy()
            .sort_values("month_start")
            .reset_index(drop=True)
        )

        hist["month_start"] = pd.to_datetime(hist["month_start"], errors="coerce")
        hist = hist.dropna(subset=["month_start"]).copy()
        ...
        if not future_df.empty:
            future_df["month_start"] = pd.to_datetime(future_df["month_start"], errors="coerce")
    ...
    def run(self, metric="visits"):
        ...
        model, pretty_model_name = self._get_model()
        model.fit(X, y)

        future_monthly = self._recursive_forecast(
            monthly_train=monthly_train,
            model=model,
            feature_cols=feature_cols,
            forecast_start_month=partial_month_start,
        )
